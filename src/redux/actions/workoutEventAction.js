...
export const createWorkoutEvent = (workoutEvent) => {
  return {
    type: WORKOUTEVENT_CREATE_WORKOUTEVENT,
...
export const updateWorkoutEvent = (workoutEvent) => {
  return {
    type: WORKOUTEVENT_UPDATE_WORKOUTEVENT,
    payload: workoutEvent,
  };
};
export const deleteWorkoutEvent = (workoutEventId) => {
  return {
    type: WORKOUTEVENT_DELETE_WORKOUTEVENT,
    payload: workoutEventId,
  };
};
export const setWorkoutEvents = (workoutEvents) => {
  return {
    type: WORKOUTEVENT_SET_ALL_WORKOUTEVENTS,
...
export const setDeleteAction = (workoutEvent) => {
  return {
    type: WORKOUTEVENT_SET_DELETE_ACTION,
    payload: workoutEvent,
  };
};
export const setUpdateAction = (workoutEvent) => {
  return {
...
export const getWorkoutEventsAsync = () => async (dispatch) => {
  try {
    ...
    if (res.status === 200) {
      ...
      for (let i = 0; i < res.data.length; i++) {
        workoutEvents.push(res.data[i]);
      }
      dispatch(setWorkoutEvents(workoutEvents));
    }
  } catch (error) {
...
export const updateWorkoutEventAsync = (workoutEvent) => async (dispatch) => {
  ...
  try {
    const res = await axios.put(
      // eslint-disable-next-line no-undef
      `${process.env.REACT_APP_BACKEND_URL_PORT}/workoutevent/${workoutEvent._id}`,
      workoutEvent,
      getConfig()
    );
    if (res.status === 200) {
